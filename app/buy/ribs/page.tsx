...
export async function generateMetadata({ searchParams }: PageProps): Promise<Metadata> {
  ...
  const canonicalPath = pageNum > 1 ? `/buy/ribs?page=${pageNum}` : "/buy/ribs";
  const title = "RIBs for Sale | Findaly";
  const description =
    "Browse RIBs and tenders for sale worldwide. Compare rigid inflatable boats, day boats and tenders on Findaly.";
...
export default async function Page({ searchParams }: PageProps) {
  ...
  const where = categoryWhere("ribs");
  ...
  return (
    <>
      <div aria-hidden="true" style={{position:"absolute",width:1,height:1,overflow:"hidden",clip:"rect(0,0,0,0)",whiteSpace:"nowrap"}}>
        <h1>RIBs for Sale — Rigid Inflatable Boats</h1>
        <p>Browse RIBs and rigid inflatable boats for sale on Findaly.</p>
        <nav>
          <a href="/">Home</a>
