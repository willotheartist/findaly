...
function buildSeoIntro(opts: {
  ...
  const bits: string[] = [];
  bits.push(
    `Browse ${total.toLocaleString()} boat${total === 1 ? "" : "s"} for sale in ${countryDisplay} on Findaly — updated regularly from trusted sellers and brokers.`
  );
    ...
    <main className="w-full bg-white">
      ...
      <section className="w-full border-b border-slate-100">
        <div className="mx-auto max-w-7xl px-4 py-10 sm:px-6 sm:py-12">
          <div className="text-xs font-semibold tracking-[0.16em] uppercase text-slate-500">Buy • Country</div>
          ...
          <div className="mt-4 text-sm text-slate-500">
            {total > 0 ? (
              ...
              <>
                No live listings found in <span className="font-semibold text-slate-700">{c.display}</span> right now.
              </>
            )}
          </div>
