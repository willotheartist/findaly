...
export async function generateMetadata({ searchParams }: PageProps): Promise<Metadata> {
  ...
  const canonicalPath = pageNum > 1 ? `/buy/catamarans?page=${pageNum}` : "/buy/catamarans";
  const title = "Catamarans for Sale | Findaly";
  const description =
    "Browse catamarans for sale worldwide. Compare cruising catamarans and performance cats with trusted listings on Findaly.";
...
export default async function Page({ searchParams }: PageProps) {
  ...
  const where = categoryWhere("catamarans");
  ...
  return (
    <>
      <div aria-hidden="true" style={{position:"absolute",width:1,height:1,overflow:"hidden",clip:"rect(0,0,0,0)",whiteSpace:"nowrap"}}>
        <h1>Catamarans for Sale — Find Your Next Catamaran</h1>
        <p>Browse catamarans for sale on Findaly. Find sailing and power catamarans from private sellers and brokers.</p>
        <nav>
          <a href="/">Home</a>
