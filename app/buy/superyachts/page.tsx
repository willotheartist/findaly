...
export async function generateMetadata({ searchParams }: PageProps): Promise<Metadata> {
  ...
  const canonicalPath = pageNum > 1 ? `/buy/superyachts?page=${pageNum}` : "/buy/superyachts";
  const title = "Superyachts for Sale | Findaly";
  const description =
    "Browse superyachts for sale worldwide. Compare large motor yachts and premium listings with trusted brokers on Findaly.";
...
export default async function Page({ searchParams }: PageProps) {
  ...
  const where = categoryWhere("superyachts");
  ...
  return (
    <>
      <div aria-hidden="true" style={{position:"absolute",width:1,height:1,overflow:"hidden",clip:"rect(0,0,0,0)",whiteSpace:"nowrap"}}>
        <h1>Superyachts for Sale — Luxury Yachts on Findaly</h1>
        <p>Browse superyachts for sale on Findaly. Find luxury motor yachts and sailing superyachts from leading brokers.</p>
        <nav>
          <a href="/">Home</a>
