...
type PageProps = {
  params: Promise<{ brand: string; country: string; year: string }>;
};
...
function buildSeoIntro(opts: {
  ...
  const { brandDisplay, countryDisplay, year, total, modelsTop = [] } = opts;
  const m = modelsTop.slice(0, 4).filter(Boolean);

  const bits: string[] = [];
  bits.push(
    `Browse ${total.toLocaleString()} ${brandDisplay} boat${total === 1 ? "" : "s"} from ${year} for sale in ${countryDisplay} on Findaly — updated regularly from trusted sellers and brokers.`
  );
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  const { brand, country, year } = await params;
  const b = brandFromParam(brand);
  const c = countryFromParam(country);
  const y = parseYearParam(year);
  if (!y) return { title: `${b.display} in ${c.display}` };

  const where = buildWhere(b, c, y);
  const total = await prisma.listing.count({ where });

  const title =
    total > 0 ? `${b.display} ${y} Boats for Sale in ${c.display}` : `${b.display} ${y} Boats in ${c.display}`;

  const description =
    total > 0
      ? truncate(
          `Browse ${total.toLocaleString()} ${b.display} boats from ${y} for sale in ${c.display}. Compare specs, prices, and locations — then enquire directly with sellers and brokers on Findaly.`,
          160
        )
      : truncate(`Explore ${b.display} boats from ${y} in ${c.display} on Findaly. New inventory is added regularly.`, 160);

  const canonical = `/buy/brand/${slugifyLoose(b.spaced)}/country/${slugifyLoose(c.spaced)}/year/${y}`;

  return {
    title,
    description,
    alternates: { canonical },
    robots: programmaticPageRobots({ listingCount: total, dimensions: 3, hasYear: true }),
    openGraph: {
      title: `${title} | Findaly`,
      description,
      url: canonical,
...
export default async function BrandCountryYearHubPage({ params }: PageProps) {
  const { brand, country, year } = await params;
  const b = brandFromParam(brand);
  const c = countryFromParam(country);
  const y = parseYearParam(year);
  ...
  const [total, listings, topModels] = await Promise.all([
  ...
  const pageUrl = `${base}/buy/brand/${safeBrandSlug}/country/${safeCountrySlug}/year/${y}`;
  ...
  const breadcrumb = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 3, name: `Brand: ${b.display}`, item: `${base}/buy/brand/${safeBrandSlug}` },
      {
        ...
        item: `${base}/buy/brand/${safeBrandSlug}/country/${safeCountrySlug}`,
      },
  ...
  const itemList = {
    "@context": "https://schema.org",
    "@type": "CollectionPage",
    name: `${b.display} ${y} Boats for Sale in ${c.display}`,
    url: pageUrl,
    description: intro,
    mainEntity: {
      "@type": "ItemList",
    ...
    <main className="w-full bg-white">
      ...
      <section className="w-full border-b border-slate-100">
        <div className="mx-auto max-w-7xl px-4 py-10 sm:px-6 sm:py-12">
          <div className="text-xs font-semibold tracking-[0.16em] uppercase text-slate-500">Buy • Brand • Country • Year</div>
          <h1 className="mt-2 text-3xl font-semibold tracking-tight text-slate-900 sm:text-4xl">
            {b.display} boats in {c.display} from {y}
          </h1>
          ...
          <div className="mt-4 text-sm text-slate-500">
            {total > 0 ? (
              ...
              <>
                No live listings found for <span className="font-semibold text-slate-700">{b.display}</span> in{" "}
                <span className="font-semibold text-slate-700">{c.display}</span> for {y}.
              </>
            )}
          </div>
          ...
          <div className="mt-8 flex flex-wrap gap-3">
            <Link
              href={`/buy/brand/${safeBrandSlug}/country/${safeCountrySlug}`}
            ...
            >
              All {b.display} boats in {c.display}
            </Link>
            <Link
            ...
            <Link
              href={`/buy/country/${safeCountrySlug}/year/${y}`}
              className="inline-flex items-center justify-center rounded-xl border border-slate-200 px-5 py-3 text-sm font-semibold text-slate-900 no-underline hover:border-slate-300"
            >
              All boats in {c.display} from {y}
            </Link>
          </div>
        </div>
      </section>
          ...
          {total === 0 ? (
            <div className="rounded-2xl border border-slate-200 bg-white p-8">
              <div className="text-lg font-semibold text-slate-900">
                No {b.display} listings live in {c.display} for {y} yet.
              </div>
              <p className="mt-2 text-slate-600">Try the brand+country page, or check back soon — inventory updates regularly.</p>
              <div className="mt-5 flex flex-wrap gap-3">
                <Link
                  href={`/buy/brand/${safeBrandSlug}/country/${safeCountrySlug}`}
                  className="inline-flex items-center justify-center rounded-xl bg-slate-900 px-5 py-3 text-sm font-semibold text-white no-underline hover:opacity-95"
                >
                  Browse {b.display} in {c.display}
                </Link>
                <Link
                  href="/add-listing"
                  className="inline-flex items-center justify-center rounded-xl border border-slate-200 px-5 py-3 text-sm font-semibold text-slate-900 no-underline hover:border-slate-300"
