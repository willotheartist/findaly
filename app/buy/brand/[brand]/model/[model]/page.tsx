...
import { slugifyLoose } from "@/lib/seoParam";
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const title = total > 0 ? `${b.display} ${m.display} Boats for Sale` : `${b.display} ${m.display} Boats`;
  ...
  const description =
    total > 0
      ...
      : truncate(`Explore ${b.display} ${m.display} boats and listings on Findaly. New inventory is added regularly.`, 160);
...
export default async function BrandModelHubPage({ params }: PageProps) {
  ...
  const faq: JsonLdObject = {
    ...
    mainEntity: [
      {
        "@type": "Question",
        name: `How much do ${b.display} ${m.display} boats cost?`,
        acceptedAnswer: {
          "@type": "Answer",
          text:
            "Pricing varies by year, size, spec, and condition. Compare live listings on this page and review photos and details before enquiring.",
        },
      },
      {
        "@type": "Question",
        name: `What years are most common for ${b.display} ${m.display}?`,
        acceptedAnswer: {
          "@type": "Answer",
          text:
            "Availability depends on the production run and generation. Use the popular years links on this page to browse listings by year.",
        },
      },
      {
        "@type": "Question",
        name: `Where are ${b.display} ${m.display} boats typically listed?`,
        acceptedAnswer: {
          "@type": "Answer",
          text:
            "Inventory clusters around major boating markets. Use the country links on this page to browse by location.",
        },
      },
      {
        "@type": "Question",
        name: "Should I get a survey before buying?",
        acceptedAnswer: {
          "@type": "Answer",
          text:
            "Yes — a pre-purchase survey helps identify structural issues, moisture ingress, and mechanical problems before you commit.",
        },
      },
      {
        "@type": "Question",
        ...
        acceptedAnswer: {
          "@type": "Answer",
          text:
            "Yes — each listing includes direct enquiry options so you can contact sellers and brokers.",
        },
      },
    ],
  };
        ...
        <div className="mx-auto max-w-7xl px-4 py-10 sm:px-6 sm:py-12">
          <div className="flex flex-col gap-6">
            <div>
              ...
              {/* Buyer guides (only routes you actually have) */}
              {safeBrandSlug === "beneteau" ? (
                <div className="mt-6 rounded-2xl border border-slate-200/80 bg-white p-5">
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      <div className="text-sm font-semibold text-slate-900">Buyer guide</div>
                      <p className="mt-1 text-sm leading-relaxed text-slate-600">
                        Research the brand, pricing, and inspection priorities before you view a boat.
                      </p>
            ...
            {(countriesTop.length > 0 || yearsTop.length > 0) && total > 0 ? (
                  ...
                  <div className="mt-3 flex flex-wrap gap-2">
                    {countriesTop.slice(0, 10).map((c) => {
                      ...
                      return (
                        <Link
                          key={c}
                          href={`/buy/brand/${safeBrandSlug}/model/${safeModelSlug}/country/${cSlug}`}
                          className="rounded-full border border-slate-200 px-3 py-1.5 text-sm text-slate-700 no-underline hover:border-slate-300 hover:text-slate-900"
                        >
                          {c}
                        </Link>
