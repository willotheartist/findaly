...
function buildWhere(
  ...
  const modelOr: Prisma.ListingWhereInput[] = [{ model: { equals: m.spaced, mode: "insensitive" } }];

  if (m.brandCandidate && m.modelCandidate) {
    modelOr.push({
      AND: [
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const title =
    total > 0 ? `${m.display} Boats for Sale in ${c.display}` : `${m.display} Boats in ${c.display}`;
  ...
  return {
    ...
    robots: programmaticPageRobots({ listingCount: total, dimensions: 3, hasYear: false }),
    openGraph: {
      title: `${title} | Findaly`,
      description,
      url: canonical,
          ...
          {total === 0 ? (
            <div className="rounded-2xl border border-slate-200 bg-white p-8">
              ...
              <p className="mt-2 text-slate-600">
                Try the model page or the country page — or check back soon.
              </p>
