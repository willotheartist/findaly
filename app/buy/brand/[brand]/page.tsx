...
/**
 * Durable brand normalization:
 * - trims
 * - lowercases
 * - collapses whitespace
 * - replaces & -> and
 * - strips punctuation
 * - strips accents/diacritics (Bénéteau -> beneteau)
 */
...
function isGarbageBrand(input: string) {
  const k = normalizeBrandKey(input);
  if (!k) return true;
...
function buildSeoIntro(opts: {
  ...
  bits.push(
    `Browse ${total.toLocaleString()} ${brandDisplay} boat${
      total === 1 ? "" : "s"
    } on Findaly — updated regularly from trusted sellers and brokers.`
  );
...
/**
 * ✅ Proper + fast:
 * Only look at brands that *roughly match* the requested brand,
 * then build exact-equals OR variants for the DB.
 *
 * Also: allow “brand+model junk” like "Beneteau Oceanis" by accepting
 * normalized keys that start with the requested key + space.
 */
async function resolveBrandVariantsForHub(b: ReturnType<typeof brandFromParam>) {
  const target = normalizeBrandKey(b.spaced);
  const targetDisplay = normalizeBrandKey(b.display);
  const targetRaw = normalizeBrandKey(b.raw);

  const keysToTry = Array.from(
    new Set([target, targetDisplay, targetRaw].filter(Boolean))
  );

  // First pass: fetch ONLY brands that contain the input in some form (cheap).
  // NOTE: This won't match diacritics reliably, but we then normalize in JS.
  const candidates = await prisma.listing.groupBy({
    by: ["brand"],
    where: {
      status: "LIVE",
      kind: "VESSEL",
      brand: { not: null },
      OR: [
        { brand: { contains: b.spaced, mode: "insensitive" } },
        { brand: { contains: b.display, mode: "insensitive" } },
        { brand: { contains: b.raw, mode: "insensitive" } },
      ],
    },
    _count: { brand: true },
  ...
  for (const row of candidates) {
    ...
    if (!raw || isGarbageBrand(raw)) continue;

    const nk = normalizeBrandKey(raw);

    // exact normalized match
    const exact = keysToTry.some((k) => nk === k);

    // allow "beneteau oceanis" to match "beneteau" hub
    const prefix = keysToTry.some((k) => nk.startsWith(`${k} `));

    if (exact || prefix) variants.push(raw);
  }

  const uniq = Array.from(new Set(variants.map((x) => x.trim()))).filter(Boolean);

  // Fallback: if no variants found, keep original tokens
  if (uniq.length === 0) {
    return Array.from(
      new Set([b.spaced, b.raw, b.display].map((x) => (x || "").trim()))
    ).filter(Boolean);
  }
...
function buildBrandWhereFromVariants(variants: string[]): Prisma.ListingWhereInput {
  return {
    status: "LIVE",
    kind: "VESSEL",
    brand: { not: null },
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const variants = await resolveBrandVariantsForHub(b);
  const where = buildBrandWhereFromVariants(variants);

  const total = await prisma.listing.count({ where });

  const title = total > 0 ? `${b.display} Boats` : `${b.display} Boats`;
  const description =
    total > 0
      ? truncate(
          `Browse ${total.toLocaleString()} ${b.display} boats worldwide. Compare specs, prices, and locations — then enquire directly with sellers and brokers on Findaly.`,
          160
        )
      : truncate(
...
export default async function BrandHubPage({ params }: PageProps) {
  ...
  const avgPriceCents = typeof agg._avg.priceCents === "number" ? Math.round(agg._avg.priceCents) : null;
  const minPriceCents = typeof agg._min.priceCents === "number" ? Math.round(agg._min.priceCents) : null;
  const maxPriceCents = typeof agg._max.priceCents === "number" ? Math.round(agg._max.priceCents) : null;
            ...
            <div>
              ...
              <div className="mt-4 text-sm text-slate-500">
                {total > 0 ? (
                  ...
                  <>
                    No live listings found for <span className="font-semibold text-slate-700">{b.display}</span> right now.
                  </>
                )}
              </div>
              ...
              {total > 0 ? (
                <div className="mt-8 rounded-2xl border border-slate-200/80 bg-white p-5">
                  <div className="flex items-center justify-between gap-3">
                    ...
                    <div className="text-xs font-semibold tracking-[0.14em] uppercase text-slate-500">Live listings</div>
                  </div>

                  <div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
                    <div className="rounded-2xl border border-slate-200/80 bg-white p-4">
                      <div className="text-xs font-semibold tracking-[0.14em] uppercase text-slate-500">Average price</div>
                    ...
                    <div className="rounded-2xl border border-slate-200/80 bg-white p-4">
                      <div className="text-xs font-semibold tracking-[0.14em] uppercase text-slate-500">Price range</div>
                      <div className="mt-1 text-xl font-semibold tracking-tight text-slate-900">
                    ...
                    <div className="rounded-2xl border border-slate-200/80 bg-white p-4">
                      <div className="text-xs font-semibold tracking-[0.14em] uppercase text-slate-500">Average length</div>
                      <div className="mt-1 text-xl font-semibold tracking-tight text-slate-900">{marketAvgLengthText}</div>
                      <div className="mt-1 text-xs text-slate-500">Uses metres when available</div>
                    </div>

                    <div className="rounded-2xl border border-slate-200/80 bg-white p-4">
                      <div className="text-xs font-semibold tracking-[0.14em] uppercase text-slate-500">Countries listed</div>
                      <div className="mt-1 text-xl font-semibold tracking-tight text-slate-900">{fmtNumber(countriesListed)}</div>
                    ...
                    {beneteauGuides.map((g) => (
                      ...
                      >
                        <div className="flex items-start justify-between gap-3">
                          <div className="text-[15px] font-semibold tracking-tight text-slate-900">{g.title}</div>
                          <div className="text-sm font-semibold text-slate-900">→</div>
                        </div>
                        <p className="mt-1.5 text-sm leading-relaxed text-slate-600">{g.desc}</p>
                        <div className="mt-3 text-xs font-semibold tracking-[0.14em] uppercase text-slate-500">Read guide</div>
                      </Link>
                    ))}
                  </div>
                ...
                <div className="rounded-2xl border border-slate-200/80 bg-white p-5">
                  ...
                  <div className="mt-3 flex flex-wrap gap-2">
                    {modelsTop.slice(0, 10).map((m) => (
                      <Link
                        key={m}
                        href={`/buy/brand/${safeBrandSlug}/model/${modelSlugFromValue(m)}`}
                        className="rounded-full border border-slate-200 px-3 py-1.5 text-sm text-slate-700 no-underline hover:border-slate-300 hover:text-slate-900"
                      >
                        {b.display} {m}
                      </Link>
