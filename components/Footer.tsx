...
function FooterLink({ href, children }: { href: string; children: React.ReactNode }) {
  return (
    <Link href={href} className="text-sm text-slate-700 no-underline hover:text-slate-900">
      {children}
    </Link>
  );
}
...
export default async function Footer() {
  ...
  const [brandAgg, modelAgg, countryAgg, yearAgg] = await Promise.all([
    ...
    prisma.listing.groupBy({
      by: ["year"],
      where: { status: "LIVE", kind: "VESSEL", intent: "SALE", year: { not: null } },
      _count: { year: true },
      ...
      <div className="w-full border-b border-slate-200 bg-white">
        <div className="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
          <div className="flex items-end justify-between gap-6">
            <div>
              <div className="text-xs font-semibold tracking-[0.16em] uppercase text-slate-500">Explore Findaly</div>
              <div className="mt-2 text-xl font-semibold tracking-tight text-slate-900">Browse popular searches</div>
              <p className="mt-2 max-w-3xl text-sm leading-relaxed text-slate-600">
                Quick links to high-intent hubs. These help buyers move faster — and help search engines discover your strongest inventory pages.
              </p>
            </div>
            ...
            <div className="lg:col-span-3">
              ...
              <div className="mt-3 grid gap-2">
                {brandsForUI.slice(0, 16).map((b) => (
                  <FooterLink key={b} href={`/buy/brand/${slugifyLoose(b)}`}>{b}</FooterLink>
                ))}
              </div>
              <div className="mt-4">
                ...
                <>
                  <div className="mt-3 grid gap-2">
                    {modelsForUI.slice(0, 16).map((m) => (
                      <FooterLink key={m} href={`/buy/model/${slugifyLoose(m)}`}>{m}</FooterLink>
                    ))}
                  </div>
                  <div className="mt-4">
            ...
            <div className="lg:col-span-3">
              ...
              <div className="mt-3 grid gap-2">
                {countriesForUI.slice(0, 16).map((c) => (
                  <FooterLink key={c} href={`/buy/country/${slugifyLoose(c)}`}>{c}</FooterLink>
                ))}
              </div>
              <div className="mt-4">
              ...
              {topYears.length ? (
                <div className="mt-3 grid gap-2">
                  {topYears.slice(0, 12).map((y) => (
                    <FooterLink key={y.year} href={`/buy/year/${y.year}`}>{y.year}</FooterLink>
                  ))}
                </div>
              ) : (
                <p className="mt-3 text-sm leading-relaxed text-slate-600">
      ...
      <div className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
        <div className="grid gap-10 lg:grid-cols-12">
          <div className="lg:col-span-4">
            <div className="text-xl font-extrabold tracking-tight text-slate-900">findaly</div>
            <p className="mt-3 max-w-sm text-sm leading-relaxed text-slate-700">
              A modern marketplace for yachts — buy, sell, and charter with confidence. Built for brokers, brands, and serious buyers.
            </p>
        ...
        <div className="mt-12 flex flex-col gap-4 border-t border-slate-200 pt-8 sm:flex-row sm:items-center sm:justify-between">
          <div className="space-y-2">
            <p className="text-xs text-slate-500">© {nowYear} Findaly. All rights reserved.</p>
